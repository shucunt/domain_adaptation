...
resultSource = os.path.join(os.path.abspath('..'), 'result/tt_tt.txt')
...
#GPU里数据类型只能是float。而data_y是类别，所以最后又转换为int返回
    ...
    test_target_set_x, test_target_set_y = shared_dataset(test_set)
    valid_target_set_x, valid_target_set_y = shared_dataset(valid_set)
    train_target_set_x, train_target_set_y = shared_dataset(train_set)

    rval = [(train_target_set_x, train_target_set_y),
            (valid_target_set_x, valid_target_set_y), (test_target_set_x, test_target_set_y)]
    return rval
...
def evaluate_lenet5(learning_rate=0.1, n_epochs=200,
                    dataset=os.path.join(os.path.abspath('..'), 'data/target0.7.pkl'),
 ...
 20个卷积核，第二个有50个
    ...
    train_target_set_x, train_target_set_y = datasets[0]
    valid_target_set_x, valid_target_set_y = datasets[1]
    test_target_set_x, test_target_set_y = datasets[2]

    # 计算batch的个数
    n_train_target_batches = train_target_set_x.get_value(borrow=True).shape[0]
    n_valid_target_batches = valid_target_set_x.get_value(borrow=True).shape[0]
    n_test_target_batches = test_target_set_x.get_value(borrow=True).shape[0]
    n_train_target_batches /= batch_size
    n_valid_target_batches /= batch_size
    n_test_target_batches /= batch_size
...
#train_model，代码分析同test_model。train_model里比test_model、validation_model多出updates规则
    train_model = theano.function(
        ...
        updates=updates,
        givens={
            x: train_target_set_x[index * batch_size: (index + 1) * batch_size],
            y: train_target_set_y[index * batch_size: (index + 1) * batch_size]
        }
    )
    ...
    validation_frequency = min(n_train_target_batches, patience / 2)
...
#iter 表示训练了多少个batch
    while (epoch < n_epochs) and (not done_looping):
        epoch = epoch + 1
        for minibatch_index in xrange(n_train_target_batches):

            iter = (epoch - 1) * n_train_target_batches + minibatch_index
...
#cost_ij 没什么用，后面都没有用到,只是为了调用train_model，而train_model有返回值
            if (iter + 1) % validation_frequency == 0:
                ...
                validation_losses = [validate_model(i) for i
                                     in xrange(n_valid_target_batches)]
                this_validation_loss = numpy.mean(validation_losses)
                print('epoch %i, minibatch %i/%i, validation error %f %%' %
                      (epoch, minibatch_index + 1, n_train_target_batches,
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    test_losses = [
                        test_model(i)
                        for i in xrange(n_test_target_batches)
                    ]
