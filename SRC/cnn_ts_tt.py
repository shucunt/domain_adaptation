...
resultSource = os.path.join(os.path.abspath('..'), 'result/ts_tt.txt')
file_w = open(resultSource, 'a')
try:
    file_w.write("theta is 0.7\n")
    file_w.write("train data is from source;valid data is from target,test data is from target\n")
...
#GPU里数据类型只能是float。而data_y是类别，所以最后又转换为int返回
    ...
    #train_target_set_x, train_target_set_y = shared_target_dataset(target_train_set)
...
def evaluate_lenet5(learning_rate=0.1, n_epochs=200,
 ...
 20个卷积核，第二个有50个
    ...
    #加载数据
    datasets = load_data(dataset_s, dataset_t)
...
def experiment(state, channel):
    evaluate_lenet5(state.learning_rate, dataset_s=state.dataset_s, dataset_t=state.dataset_t)
